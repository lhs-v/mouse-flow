...
**설정** 탭을 위에서부터 순서대로 따라가면 된다.

1. 권한 허용
2. 키보드·마우스 선택 (둘 다 폰에 연결된 상태에서)
3. PC 가 마우스의 몇 번 채널인지 선택
4. **[자동 설정]** — 마우스를 폰 채널에 켜 둔 상태에서. feature index 와 현재 호스트를 읽어온다
5. 배터리 최적화 해제

**상태** 탭에서 **[마우스를 PC 로 보내기]** 로 수동 확인한 뒤 **자동 전환 사용** 을 켠다.

프로토콜 수준 진단은 **고급** 탭에 있다.
