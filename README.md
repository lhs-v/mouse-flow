...
> **검증 상태**
>
> - **PowerShell**: 코드 경로 전체가 실제로 실행 확인됨 — Add-Type 컴파일, HID 열거,
>   `CreateFile`, 리포트 송신, 오버랩드 수신, 오류 처리까지. 다만 **로지텍 HID++ 기기가
>   없는 환경에서 검증**했으므로 ChangeHost 왕복 자체는 아직 미확인이다.
> - **안드로이드**: 아직 컴파일해 보지 않았다. GATT 경로는 이론적 근거만 있고 **미검증**이다.
>
